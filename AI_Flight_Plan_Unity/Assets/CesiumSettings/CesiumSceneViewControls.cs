...
public class CesiumSceneViewControls : MonoBehaviour
{
    ...
    [Header("Pan")]
    public float panSensitivity = 1.0f;   // 1 = doðal; daha hýzlý/yavaþ için ayarla
    public bool invertPan = false;        // harita hissi için genelde false
    public bool clampLatLon = true;       // kutup/meridyen sapmalarýný engelle

    [Header("Zoom")]
    public bool zoomEnabled = true;
    public float zoomSpeed = 1.2f;        // >1 log ölçek (1.1–1.4 iyi)
    ...
    const double EarthRadius = 6378137.0; // WGS84 (yaklaþýk)
    ...
    void Awake()
    {
        ...
        _lonLatH = globeAnchor ? globeAnchor.longitudeLatitudeHeight : new double3(29.0, 41.0, 100.0); // Ýstanbul civarý default
    }
    ...
    void Update()
    {
        ...
        // UI üzerinde iken kontrolleri kapat
    ...
    void HandlePan()
    {
        if (Input.GetKeyDown(panMouse))
        {
            ...
            SetOrbit(false); // pan sýrasýnda orbit kapansýn
        }
        ...
        if (metersPerDegLon < 1e-6) metersPerDegLon = 1e-6; // kutuplarda patlamasýn
        ...
        if (clampLatLon)
        {
            // Lon’u [-180,180) aralýðýna getir
            _lonLatH.x = WrapLon(_lonLatH.x);
            // Lat’ý [-85,85] gibi makul bir banda kýs
    ...
    void HandleZoom()
    {
        ...
        // Logaritmik ölçek: scroll>0 yakýnlaþ, <0 uzaklaþ
