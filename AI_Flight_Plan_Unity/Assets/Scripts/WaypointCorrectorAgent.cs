...
{
    public BSplineSegment bSplineDrawer;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        Vector3 pos = bSplineDrawer.controlPoint1.localPosition + new Vector3(moveX, 0f, moveZ) * Time.deltaTime;
        float posX = Mathf.Clamp(pos.x, -10, 10);
        float posZ = Mathf.Clamp(pos.z, -5, 5);
        bSplineDrawer.controlPoint1.localPosition = new Vector3(posX, 0f, posZ);


        moveX = actions.ContinuousActions[2] * moveScale;
        moveZ = actions.ContinuousActions[3] * moveScale;
        pos = bSplineDrawer.controlPoint2.localPosition + new Vector3(moveX, 0f, moveZ) * Time.deltaTime;
        posX = Mathf.Clamp(pos.x, -10, 10);
        posZ = Mathf.Clamp(pos.z, -5, 5);
        bSplineDrawer.controlPoint2.localPosition = new Vector3(posX, 0f, posZ);
    ...
    { 
        bSplineDrawer.startPoint.localPosition = new Vector3(-13f,0f,0f);
        bSplineDrawer.endPoint.localPosition = new Vector3(+13f,0f,0f);
        ...
        bSplineDrawer.controlPoint1.localPosition = new Vector3(randX, 0f, randZ);

        randX = Random.Range(0f, +10f);
        randZ = Random.Range(-10f, +10f);
        bSplineDrawer.controlPoint2.localPosition = new Vector3(randX, 0f, randZ);

        episodeStartTime = Time.time; // simülasyon zamaný (timeScale'dan etkilenir)
    }
    public override void CollectObservations(VectorSensor sensor)
    {
        sensor.AddObservation(new Vector2(bSplineDrawer.controlPoint1.localPosition.x, bSplineDrawer.controlPoint1.localPosition.z) / 10f);
        sensor.AddObservation(new Vector2(bSplineDrawer.controlPoint2.localPosition.x, bSplineDrawer.controlPoint2.localPosition.z) / 10f);
